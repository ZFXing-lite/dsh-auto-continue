 ...
 *  4. Injection is GESTURE-DRIVEN, not observer-driven. The menu only opens in
 *     response to a user gesture on the "+" button, so we listen (capture phase)
 *     for `click`/`keydown` on that button and inject right after React commits.
 *     The menu unmounts on close, taking our node with it; the next open is a new
 *     gesture, which injects again. A render-signature guard makes `render()`
 *     idempotent, so a duplicate schedule never rebuilds the row under the pointer.
 ...
 *  6. NO global MutationObserver. An earlier version observed
 *     `document.documentElement` with `{subtree:true}`, which fired on every DOM
 *     mutation across the whole page (every streaming token, every React
 *     reconciliation) and ran `render()` every animation frame even with the menu
 *     closed — that was the cause of the severe UI lag when auto-continue kept
 *     turns streaming back-to-back. A tiny scoped observer is armed only for a
 *     short window after a "+" gesture, on the one composer card the user touched,
 *     to catch any async menu mount; it disarms itself, so steady-state cost is
 *     zero while the menu is closed.
 *
 * ── State ─────────────────────────────────────────────────────────────────
 *
...
`;
		...
		function apply(ctx) {
			...
			/* ── render scheduling ───────────────────────────────────────────────
			 * render() walks the DOM, so we coalesce calls through a single
			 * requestAnimationFrame. Crucially we do NOT keep a permanent
			 * MutationObserver: an earlier version observed documentElement with
			 * {subtree:true}, which fired on every mutation across the whole page
			 * (every streaming token, every React reconciliation) and ran render()
			 * every frame even with the menu closed — the cause of the severe lag
			 * when auto-continue kept turns streaming back-to-back. The "+" menu
			 * only opens on a user gesture, so we drive injection from those
			 * gestures and arm a tiny scoped observer only briefly around an open.
			 * Steady-state cost is zero: nothing runs while the menu is closed. */
			let queued = false;
			const schedule = () => {
				if (queued) return;
				queued = true;
			...
			/* Two frames after a gesture: React commits the menu on the click task,
			 * so by the second rAF the [role="menu"] node is in the tree. */
			const scheduleAfterGesture = () => {
				(typeof requestAnimationFrame === "function" ? requestAnimationFrame : window.setTimeout)(schedule);
			};

			/* ── scoped safety-net observer ──────────────────────────────────────
			 * Armed only for a short window after a "+" gesture, on the ONE composer
			 * card the user touched — never on the whole document. It catches a menu
			 * that mounts a tick later than the gesture (portals / concurrent mode)
			 * and then disarms itself. While the menu is closed this observer does
			 * not exist, so streaming output costs nothing here. */
			...
			const disarmScoped = () => {
				...
				if (scopedTimer !== null) {
					window.clearTimeout(scopedTimer);
					scopedTimer = null;
				}
			};
			...
			/* ── gesture-driven injection ────────────────────────────────────────
			 * Capture-phase delegation: any click or key gesture that lands on (or
			 * inside) a composer "+" button opens its menu, so we schedule an inject
			 * for right after React commits. A gesture that is not on a "+" button
			 * is ignored entirely — no query, no render, no frame spent. */
			const ADD_BTN_SELECTOR = '[data-composer-card] button[aria-haspopup="menu"]';
